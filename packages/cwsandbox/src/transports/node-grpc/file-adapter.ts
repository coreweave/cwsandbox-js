...
export function createGrpcFileAdapter(prepare: PrepareDataPlaneCall): FileAdapter {
  return {
    read: (request) => grpcReadFile(prepare, request),
    write: (request) => grpcWriteFile(prepare, request),
    readStream: (request) => grpcReadStream(prepare, request),
    writeStream: (request) => grpcWriteStream(prepare, request),
  };
}

async function grpcWriteFile(
  prepare: PrepareDataPlaneCall,
  request: WriteFileRequest,
): Promise<void> {
  const prepared = await prepare(request.sandboxId, SandboxDataPermission.WRITE_FILE, request);
  try {
    await withGrpcErrorMapping(
      "Write file",
      () =>
        prepared.client.writeFile(
          ProtoWriteFileRequest.create({
            content: request.content,
            path: request.path,
            sandboxId: request.sandboxId,
          }),
          { ...prepared.rpcOptions, ...toRpcOptions(request) },
        ).response,
...
async function grpcReadFile(
  ...
  const prepared = await prepare(request.sandboxId, SandboxDataPermission.READ_FILE, request);
  try {
    const response = await withGrpcErrorMapping(
      "Read file",
      () =>
        prepared.client.readFile(
...
function grpcReadStream(
  ...
  return {
    [Symbol.asyncIterator]() {
      return grpcReadStreamIterator(prepare, request);
    },
  };
}
...
async function* grpcReadStreamIterator(
  ...
  const prepared = await prepare(request.sandboxId, SandboxDataPermission.STREAM_EXEC, request);
  const streamingClient = prepared.client;
  try {
