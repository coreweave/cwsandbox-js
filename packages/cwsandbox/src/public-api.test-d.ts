...
test("public API types", async () => {
  ...
  expectTypeOf(sandbox.files.write(fileWritesArray, { timeoutMs: 1 })).toEqualTypeOf<
    Promise<void>
  >();
  ...
  expectTypeOf(client.deleteSnapshot("snap-123", { missingOk: true })).toEqualTypeOf<
    Promise<void>
  >();
  expectTypeOf(client.getSnapshot("snap-123")).toEqualTypeOf<Promise<FileSystemSnapshotResult>>();
  expectTypeOf(client.listSnapshots()).toEqualTypeOf<
