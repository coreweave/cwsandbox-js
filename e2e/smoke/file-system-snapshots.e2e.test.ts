...
const NAMED_MOUNT_PATH = "/data";
const NAMED_VOLUME = "data";
...
describeWithCredentials("live file-system snapshot smoke", { sequential: true }, () => {
  ...
  it(
    "snapshots and restores a named scratch volume that is not workspace",
    async () => {
      const payload = `hello from named volume smoke ${randomUUID()}`;
      const snapshotIds: string[] = [];
      ...
      try {
        ...
        await withDedicatedTaggedSandbox(
          ...
          async (source) => {
            sourceSandboxId = source.sandboxId;
            console.log(`Started named-volume sandbox: ${source.sandboxId}`);

            const write = await source.exec([
              "sh",
              "-c",
              `echo '${payload}' > ${NAMED_MOUNT_PATH}/data.txt`,
            ]);
            logProcessResult("seed named scratch mount", write);
            expect(write.exitCode).toBe(0);
            ...
            expect(snapshot.snapshotId).toMatch(/\S/);
            expect(snapshot.state).toBe("ready");
            console.log("created named-volume snapshot", {
              objectBucket: snapshot.objectBucket,
              snapshotId: snapshot.snapshotId,
              sizeBytes: snapshot.sizeBytes,
              sourceSandboxId: source.sandboxId,
              sourceVolumeName: snapshot.sourceVolumeName,
              state: snapshot.state,
              trigger: snapshot.trigger,
            });
          },
        ...
        await withDedicatedTaggedSandbox(
          ...
          async (restored) => {
            console.log(`Started restored named-volume sandbox: ${restored.sandboxId}`);
            expect(restored.sandboxId).not.toBe(expectedSourceSandboxId);

            const read = await restored.exec(["cat", `${NAMED_MOUNT_PATH}/data.txt`]);
            logProcessResult("restored named scratch mount", read);
            expect(read.exitCode).toBe(0);
            expect(read.stdout).toContain(payload);
          },
        );
